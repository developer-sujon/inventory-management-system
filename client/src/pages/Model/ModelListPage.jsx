...
const ModelListPage = () => {
  ...
  const { ModelLists, TotalModel } = useSelector((state) => state.Model);

  const PerPageOnChange = (e) => {
    if (e.target.value === "All") {
      setPerPage(TotalModel);
    } else {
          ...
          <Card>
            <Card.Body>
              <Row className="mb-2">
                <Col sm={5}>
                  <Link
                    to="/model/model-create-update"
                    className="btn btn-danger mb-2"
                  >
                    <i className="mdi mdi-plus-circle me-2"></i> Add Model
                  </Link>
                </Col>
                ...
                <Col sm={7}>
                  <div className="text-sm-end">
                    ...
                    <Button
                      variant="light"
                      className="mb-2 me-1"
                      onClick={() => ExportDataJSON(ModelLists, "Model", "xls")}
                    >
                    ...
                    <Button
                      variant="light"
                      className="mb-2"
                      onClick={() => ExportDataJSON(ModelLists, "Model", "csv")}
                    >
                ...
                <Col>
                  <div className="mb-2">
                    <span className="d-flex align-items-center">
                      Search :{" "}
                      <input
                        placeholder={TotalModel + " records..."}
                            ...
                            <td>
                              <span
                                className={classNames("badge", {
                                  "bg-success": record?.ModelStatus,
                                  "bg-danger": !record?.ModelStatus,
                                })}
                              >
                                {record?.ModelStatus ? "Active" : "Deactivated"}
                              </span>
                            </td>
                            <td>
                              <Link
                                to={`/Model/Model-create-update?id=${record?._id}`}
                ...
                <Col>
                  <div className="d-lg-flex align-items-center text-center pb-1">
                    ...
                    <span className="me-3">
                      Page
                      <strong>
                        {pageNumber} of {Math.ceil(TotalModel / perPage)}
                      </strong>
                    </span>
