...
const ProductListPage = () => {
  ...
  useEffect(() => {
    ProductRequest.ProductList(pageNumber, perPage, searchKey);
  }, [pageNumber, perPage, searchKey]);

  const { ProductLists, TotalProduct } = useSelector((state) => state.Product);

  const PerPageOnChange = (e) => {
    if (e.target.value === "All") {
      setPerPage(TotalProduct);
    } else {
  ...
  const DeleteProduct = (id) => {
    AleartMessage.Delete(id, ProductRequest.ProductDelete).then((result) => {
      if (result) {
        ProductRequest.ProductList(pageNumber, perPage, searchKey);
      }
    });
  ...
  return (
    <>
      <PageTitle
        ...
        title={"Product List " + TotalProduct}
      />
      <Row>
        <Col xs={12}>
          <Card>
            <Card.Body>
              <Row className="mb-2">
                <Col sm={5}>
                  <Link
                    to="/Product/Product-create-update"
                ...
                <Col sm={7}>
                  <div className="text-sm-end">
                    ...
                    <Button
                      ...
                      className="mb-2 me-1"
                      onClick={() =>
                        ExportDataJSON(ProductLists, "Product", "xls")
                      }
                    >
                    ...
                    <Button
                      ...
                      className="mb-2"
                      onClick={() =>
                        ExportDataJSON(ProductLists, "Product", "csv")
                      }
                    >
                ...
                <Col>
                  <div className="mb-2">
                    <span className="d-flex align-items-center">
                      Search :{" "}
                      <input
                        placeholder={TotalProduct + " records..."}
                      ...
                      {ProductLists?.map((record, index) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>
                              <span
                                className={classNames("badge", {
                                  "bg-success": record?.ProductStatus,
                                  "bg-danger": !record?.ProductStatus,
                                })}
                              >
                            ...
                            <td>
                              <Link
                                to={`/Product/Product-create-update?id=${record?._id}`}
                ...
                <Col>
                  <div className="d-lg-flex align-items-center text-center pb-1">
                    ...
                    <span className="me-3">
                      Page
                      <strong>
                        {pageNumber} of {Math.ceil(TotalProduct / perPage)}
                      </strong>
                    </span>
