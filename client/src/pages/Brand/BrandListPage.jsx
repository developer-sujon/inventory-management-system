...
const BrandListPage = () => {
  ...
  const { BrandLists, TotalBrand } = useSelector((state) => state.Brand);

  const PerPageOnChange = (e) => {
    if (e.target.value === "All") {
      setPerPage(TotalBrand);
    } else {
                ...
                <Col sm={7}>
                  <div className="text-sm-end">
                    ...
                    <Button
                      variant="light"
                      className="mb-2 me-1"
                      onClick={() => ExportDataJSON(BrandLists, "Brand", "xls")}
                    >
                    ...
                    <Button
                      variant="light"
                      className="mb-2"
                      onClick={() => ExportDataJSON(BrandLists, "Brand", "csv")}
                    >
                ...
                <Col>
                  <div className="mb-2">
                    <span className="d-flex align-items-center">
                      Search :{" "}
                      <input
                        placeholder={TotalBrand + " records..."}
                            ...
                            <td>
                              <span
                                className={classNames("badge", {
                                  "bg-success": record?.BrandStatus,
                                  "bg-danger": !record?.BrandStatus,
                                })}
                              >
                                {record?.BrandStatus ? "Active" : "Deactivated"}
                              </span>
                            </td>
                            <td>
                              <Link
                                to={`/brand/brand-create-update?id=${record?._id}`}
                ...
                <Col>
                  <div className="d-lg-flex align-items-center text-center pb-1">
                    ...
                    <span className="me-3">
                      Page
                      <strong>
                        {pageNumber} of {Math.ceil(TotalBrand / perPage)}
                      </strong>
                    </span>
